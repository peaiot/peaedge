...
func SaveModbusRegRtd(regId string, result string, errstr string) {
	...
	if errstr == "" || errstr == "success" {
		err = gormDB.Exec("update modbus_reg set rtd = ?, err_times = 0, last_error= 'success'  where id = ?",
			result, regId).Error
	} else {
		err = gormDB.Exec("update modbus_reg set rtd = ?, err_times = IFNULL(err_times,0) +1, last_error= ?  where id = ?",
