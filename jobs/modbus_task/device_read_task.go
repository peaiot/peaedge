...
func _readCoilData(dev models.ModbusDevice, reg models.ModbusReg, client modbus.Client) {
	...
	app.SaveModbusRegRtd(reg.Id, strconv.Itoa(int(val)), "success")
}
...
func _read0304RegisterData(dev models.ModbusDevice, reg models.ModbusReg, client modbus.Client, regtype string) {
	...
	if val == ERRORVALUE {
		...
		app.SaveModbusRegRtd(reg.Id, "N/A", "ERRORVALUE")
		return
	}
	...
	if result == "" {
		...
		app.SaveModbusRegRtd(reg.Id, "N/A", "NoResult")
		return
	}

	app.SaveModbusRegRtd(reg.Id, result, "success")
}
